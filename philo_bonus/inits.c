...
/*   Updated: 2025/04/19 13:26:44 by asoudani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int philo_init(t_data *data)
{
	...
	if (!data->philos)
		exitWithMessage("Malloc error", ERROR);
...
/*
	when opening a semaphore, it's important to unlink the pre-existed one under the same name
...
int data_init(t_data *data, char **av)
{
	...
	data->forks = sem_open("/philo_forks", O_CREAT | O_EXCL, 0644, data->number_of_philos);
	data->quiter = sem_open("/philo_quiter", O_CREAT | O_EXCL, 0644, 1);
	data->print_lock = sem_open("/philo_print_lock", O_CREAT | O_EXCL, 0644, 1);
	if (data->forks == SEM_FAILED || data->quiter == SEM_FAILED || data->print_lock == SEM_FAILED)
		exitWithMessage("Semaphore creation error", ERROR);
